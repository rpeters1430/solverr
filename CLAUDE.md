...
- `POST /mcp` (Streamable HTTP, `app/mcp_server.py`) — MCP server exposing `solverr_scrape`/`solverr_screenshot`/`solverr_get_cookies`/`solverr_get_stats` tools for AI agents; on by default, disable with `ENABLE_MCP=false`. Subject to the same `X-Api-Key` gate as every other route. `stateless_http=True` (no sticky sessions needed across replicas) and DNS-rebinding protection disabled (that check defends a localhost-bound desktop MCP server against browser-based DNS rebinding, which doesn't apply to Solverr's network-exposed, key-gated deployment model).
