...
- `POST /mcp` (Streamable HTTP, `app/mcp_server.py`) — MCP server exposing `solverr_scrape`/`solverr_screenshot`/`solverr_get_cookies`/`solverr_get_stats` tools for AI agents; on by default, disable with `ENABLE_MCP=false`. Subject to the same `X-Api-Key` gate as every other route. `stateless_http=True` (no sticky sessions needed across replicas). DNS-rebinding/Host-header protection (`_mcp_transport_security()`) is disabled when `API_KEY` is set (the shared secret is the real gate then) but stays **on and localhost-only** when it isn't — otherwise an unauthenticated MCP surface (`solverr_get_cookies` included) would be reachable from any browser tab via DNS rebinding. Widen it for a real non-localhost, no-`API_KEY` deployment via `MCP_ALLOWED_HOSTS`/`MCP_ALLOWED_ORIGINS`.
...
2. **Tier 2 — Clearance Cookie Cache** (`app/solver/cache.py`): Reuses valid `cf_clearance` tokens and domain cookie jars. Supports zero-dependency local disk JSON storage (writes are debounced/throttled off the request hot path) or distributed Redis via `REDIS_URL`. Both `CookieCache` and `SessionManager` (`app/solver/sessions.py`) retry a failed Redis connection on a cooldown (`REDIS_RECONNECT_INTERVAL_SECONDS`, 30s) via their `_redis()` helper instead of disabling Redis permanently for the process's lifetime — recovers on its own from a Redis container that wasn't ready yet when this one started (e.g. `docker compose --profile distributed up`), without a restart.
3. **Tier 3 — Multi-WAF Stealth Browser** (`app/solver/browser/` + `app/solver/human_cursor.py`): Camoufox (Stealth Firefox) with cubic Bézier human mouse curves to defeat Turnstile, reCAPTCHA v2, hCaptcha, GeeTest, Imperva, DataDome, Akamai, and AWS WAF. `BrowserPool.solve()` (`app/solver/browser/browser.py`) tries a warm pooled instance first (`CamoufoxPool` in `app/solver/browser/pool.py` — a bounded pool of warm, no-proxy browser processes reused across solves, a fresh context per request rather than a fresh process, recycled after `CAMOUFOX_POOL_RECYCLE_USES`/`_SECONDS`), then always retries once on a fresh ephemeral Camoufox instance (new randomly generated fingerprint) before giving up — either as the pooled path's retry, or as the only attempt for requests carrying their own proxy or an explicit `userAgent` (which bypass the pool entirely, since Camoufox ties fingerprint/geo derivation to the proxy at launch time). `app/solver/browser/__init__.py` re-exports the package's public API (`browser_pool`, `BrowserPool`, `CamoufoxPool`, `detect_challenge`, etc.) so `from app.solver.browser import ...` call sites are unaffected by the internal module split (see "Module layout" below).
...
- `app/main.py` — FastAPI app, lifespan (periodic session cleanup; the Camoufox pool itself warms up lazily on first solve, not at startup; enters the MCP session manager's `run()` context when `ENABLE_MCP` is set — mounting a Starlette sub-app alone does not invoke its lifespan, so this is wired explicitly via `contextlib.AsyncExitStack`), request-logging/auth middleware, `/health` and `/metrics`.
- `app/api/flaresolverr.py` — `/v1`, `/v2`, `/scrape`, `/proxy` route handlers.
- `app/mcp_server.py` — the MCP server (`MCPServer` from the `mcp` package, mounted at `/mcp` by `app/main.py`). Tools call straight into the same `solver_engine`/`cookie_cache`/`browser_pool` singletons the HTTP routes use, so an MCP-driven solve shows up in `/metrics` and the dashboard like any other request. `create_mcp_asgi_app()` must run before `mcp_server.session_manager` is accessible (building the ASGI app is what constructs the session manager) — see `app/main.py`'s module-level `mcp_asgi_app` for the exact ordering.
...
- `app/security.py` — SSRF protection via `check_target_url_async()` (DNS resolved on the event loop; a sync `check_target_url()` remains for older callers). Blocks loopback/private/link-local/reserved/multicast addresses plus `localhost` and cloud-metadata hostnames by default (`ALLOW_PRIVATE_NETWORKS=false`), with `ALLOWED_HOSTS`/`DENIED_HOSTS` overrides; a host that fails to resolve is blocked (fail-closed). Applied at every egress point:
  - `HybridSolverEngine.process_request` checks the initial target **and** the request's `proxy` URL (the proxy is the real egress point), covering `/v1`, `/v2`, `/scrape`, `/proxy`, and MCP uniformly.
  - Tier 1 (`fast_tls.py`) follows redirects manually (`allow_redirects=False`, up to `MAX_REDIRECTS=10`) and re-checks every `Location` hop.
  - Tier 3 (`browser/navigation.py`'s `install_media_blocking` route) checks every browser request, including redirects and subresources, and aborts blocked ones.
- `app/solver/browser/` — Tier 3 engine package, split by responsibility: `pool.py` (Camoufox process lifecycle), `models.py` (`_PooledCamoufox`), `challenges.py` (pure WAF/age-gate detection, no page dependency — directly unit-tested in `tests/test_challenge_detection.py`), `captcha.py` (paid captcha-solver escalation: sitekey extraction, token injection), `cookies.py` (Playwright↔`CookieModel` conversion, unit-tested in `tests/test_browser_cookie_and_nav_helpers.py`), `navigation.py` (GET/POST page navigation, media-blocking route), `interactions.py` (human-like challenge-widget click dispatch), and `browser.py` (`BrowserPool` — ties the rest into the actual solve workflow: `solve()` → `_solve_with_pooled_camoufox`/`_solve_with_ephemeral_camoufox` → `_execute_solve_flow`). `__init__.py` re-exports the full prior single-file public API, so no other module's imports changed when this was split out of a single 1091-line `browser.py` (2026-09-03).

Optional API-key auth (`API_KEY` env var) is enforced in `app/main.py`'s middleware for every route except `/health`, `/health/live`, `/health/ready`, `/static/*`, `/favicon.ico`, and `/metrics` (the last only while `METRICS_REQUIRE_AUTH=false`). The key is accepted via `X-Api-Key` or `Authorization: Bearer`, compared in constant time, and deliberately never via query string (it would leak into logs and Referer headers).
...
1. **`deps` stage** — creates a venv, `uv pip install -r requirements.txt` (uv, not pip, for faster/reproducible dependency resolution — see the Dockerfile's `deps` stage comments), then pre-fetches a pinned Camoufox browser engine (`python -m camoufox fetch "official/stable/${CAMOUFOX_BROWSER_VERSION}"`, set via the `CAMOUFOX_BROWSER_VERSION` build arg so a version bump invalidates that layer) so it's baked into the image rather than downloaded at container start. Trims it afterward: deletes the `fonts/macos` and `fonts/windows` sets Camoufox also downloads by default (safe only because `app/solver/browser/pool.py`/`browser.py` pin `os="linux"` on every launch — Camoufox otherwise randomizes fingerprint OS per launch and needs all three font sets to render faithfully for whichever it picks), and `strip --strip-unneeded`s the Firefox binary/shared libs. Together with a curated (not `playwright install-deps firefox`) apt list in the runtime stage, this took the image from 3.23GB to 1.42GB.
...
Container startup: `tini` is PID 1 (`ENTRYPOINT`), invoking `docker-entrypoint.sh`, which then `exec`s `python -m app.main` (the `CMD`). The container runs as **root by default** — set `PUID`/`PGID` together (both required, both must be numeric) to have the entrypoint `chown` only the runtime-writable paths (`/app/data`, `/app/home`, `/app/.cache/camoufox/tmp`, `/app/.cache/camoufox/fontconfig`) to that UID/GID and re-exec the process under it via `gosu`. The Camoufox browser tree itself is deliberately not chowned — doing so on every start forces overlayfs to copy it into the container's writable layer. Setting only one of the pair fails the container at startup by design. The entrypoint also registers a matching `/etc/passwd`/`/etc/group` entry (home `/app/home`) for an unrecognized PUID/PGID before dropping privileges, since `gosu` derives `$HOME` from the target UID's passwd entry (ignoring any exported `HOME`) — an unregistered UID would otherwise resolve to `HOME=/`, which is read-only for a non-root user and breaks Camoufox's config/cache directories.
...
`HEALTHCHECK` and the compose `healthcheck:` block poll `GET /health` via `python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8191/health', timeout=3)"`. `curl` is also installed in the runtime image to ensure compatibility with user compose overrides, container platforms (Portainer, CasaOS, Unraid), and legacy healthchecks that execute `curl`. `/health` in `app/main.py` reports `503` only when the Camoufox import itself failed (`CAMOUFOX_AVAILABLE` is False) — there's no other engine left to service Tier 3 in that case. An unused/lazy pool (nothing solved yet) is healthy.
