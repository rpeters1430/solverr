...
class TestMCPServer(unittest.TestCase):
    # Class-level client: the MCP session manager's run() can only be entered once per process.
    @classmethod
    def setUpClass(cls):
        # Without API_KEY, MCP only accepts localhost; the default "testserver" Host would get a 421.
    ...
    def test_get_cookies_reads_the_shared_cache_without_a_network_request(self):
        ...
        try:
            ...
            # The global cache persists to disk, so the removal must be saved too.
    ...
    def test_scrape_failure_does_not_leak_the_raw_exception_to_the_caller(self):
        # Exception text can carry proxy credentials, so only a sanitized message may reach the client.
    ...
    def test_untrusted_host_is_rejected_without_an_api_key(self):
        # No API_KEY here, so a non-local Host must be rejected.
